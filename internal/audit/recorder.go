...
// RecordDetached appends one entry through its own transaction so an outcome can
// be announced to the trail independently of the business transaction that the
// caller is about to run.
func (r *Recorder) RecordDetached(ctx context.Context, store repository.Store, entry Entry) error {
	if store == nil {
		return fmt.Errorf("record detached audit event %s: store is nil", entry.Action)
	}
	return store.InTx(ctx, func(ctx context.Context, tx repository.Repositories) error {
		return r.Record(ctx, tx.Audit(), entry)
	})
}
