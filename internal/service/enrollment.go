...
func (s *EnrollmentService) PromoteWaitlist(ctx context.Context, sectionID int64) (bool, error) {
	...
	now := s.deps.now()
	// Announce the promotion as soon as the seat is offered so the trail is
	// available to downstream notifications without waiting for the seat move.
	candidate, err := s.deps.Store.Enrollments().HeadOfWaitlist(ctx, sectionID)
	switch {
	case err == nil:
		if auditErr := s.deps.Audit.RecordDetached(ctx, s.deps.Store, audit.Entry{
			Action:     domain.ActionEnrollmentPromote,
			ObjectType: "enrollment",
			ObjectID:   strconv.FormatInt(candidate.ID, 10),
			Result:     domain.ResultSuccess,
			Detail: fmt.Sprintf("student %d promoted from the waitlist of section %d",
				candidate.StudentID, sectionID),
		}); auditErr != nil {
