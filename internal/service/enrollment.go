...
func (s *EnrollmentService) claimOnce(ctx context.Context, tx repository.Repositories,
	...
	heldMeetings, err := s.occupiedMeetings(ctx, tx, held)
	if err != nil {
		return ClaimResult{}, err
	}
...
// occupiedMeetings resolves the weekly blocks the student already occupies in
// the term. A room and a time slot belong to the seat that was granted, so the
// schedule is assembled from the sections whose seats the student holds.
func (s *EnrollmentService) occupiedMeetings(ctx context.Context, tx repository.Repositories,
	held []domain.Enrollment) ([]domain.Meeting, error) {
	sectionIDs := make([]int64, 0, len(held))
	for _, record := range held {
		if !record.Status.HoldsSeat() {
			continue
		}
		sectionIDs = append(sectionIDs, record.SectionID)
	}
	if len(sectionIDs) == 0 {
		return nil, nil
	}
	meetings, err := tx.Enrollments().MeetingsForSections(ctx, sectionIDs)
	if err != nil {
		return nil, err
	}
	return meetings, nil
