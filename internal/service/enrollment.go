...
func (s *EnrollmentService) Drop(ctx context.Context, actor domain.Principal, enrollmentID int64, reason string) (domain.Enrollment, error) {
	...
	err := s.deps.Store.InTx(ctx, func(ctx context.Context, tx repository.Repositories) error {
		...
		if domain.SeatDelta(previous, target) < 0 {
			...
			if released.WaitlistLength > 0 {
				// The queue has to be served. Remember the section and enqueue the
				// job after this transaction, so the durable queue write does not
				// have to wait behind the seat write lock.
	...
	if promotionSectionID > 0 {
		if enqueueErr := s.enqueueWaitlistPromotion(ctx, promotionSectionID, now); enqueueErr != nil {
			return domain.Enrollment{}, errors.Join(err, enqueueErr)
		}
	}
	if err != nil {
...
func (s *EnrollmentService) enqueueWaitlistPromotion(ctx context.Context, sectionID int64, now time.Time) error {
