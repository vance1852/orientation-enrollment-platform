...
type IdempotencyService struct {
	deps         Deps
	reservations *idempotencyReservations
}
...
func NewIdempotencyService(deps Deps) (*IdempotencyService, error) {
	...
	return &IdempotencyService{deps: deps, reservations: newIdempotencyReservations()}, nil
}
...
// reservationKey identifies the endpoint and client supplied key of a request.
func (s Scope) reservationKey() string {
	return s.Method + "|" + s.Path + "|" + s.Key
}
...
func (s *IdempotencyService) Execute(ctx context.Context, scope Scope, fn func(ctx context.Context) (int, string, error)) (Outcome, error) {
	...
	if reserved, ok := s.reservations.load(scope.reservationKey(), s.deps.now()); ok {
		...
		return Outcome{Status: reserved.status, Body: reserved.body, Replayed: true}, nil
	}
	...
	s.reservations.store(scope.reservationKey(), idempotencyReservation{
