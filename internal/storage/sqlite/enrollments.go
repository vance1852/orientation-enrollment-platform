...
// StudentPlanRecords returns the study plan a new seat request is evaluated
// against, ordered by request time so the duplicate and credit rules always see
// the records in the order the student created them.
func (d *dataset) StudentPlanRecords(ctx context.Context, studentID int64) ([]domain.Enrollment, error) {
	return d.queryEnrollments(ctx, `
        SELECT `+enrollmentColumns+` FROM enrollments
        WHERE student_id = ? AND status IN ('pending', 'enrolled', 'waitlisted')
        ORDER BY requested_at, id`, studentID)
}
