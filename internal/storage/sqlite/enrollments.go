...
// MeetingsForSections returns the weekly blocks of the given sections. The
// caller supplies the sections it already resolved, so this query does not
// re-derive the student's participation from the enrollment table.
func (d *dataset) MeetingsForSections(ctx context.Context, sectionIDs []int64) ([]domain.Meeting, error) {
	if len(sectionIDs) == 0 {
		return nil, nil
	}
	...
	rows, err := d.q.QueryContext(ctx, `
        SELECT id, section_id, weekday, start_minute, end_minute, room
        FROM section_meetings
        WHERE section_id IN (`+strings.Join(placeholders, ",")+`)
        ORDER BY weekday, start_minute, id`, args...)
	if err != nil {
		return nil, fmt.Errorf("list meetings of %d sections: %w", len(sectionIDs), err)
	}
	defer func() { _ = rows.Close() }()

	meetings := make([]domain.Meeting, 0, len(sectionIDs)*2)
	for rows.Next() {
		var (
			meeting domain.Meeting
			weekday int
